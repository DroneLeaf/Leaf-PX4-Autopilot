...
private:
	...
	bool send() override
	{
		if (_actuator_armed_sub.updated()) {
			...
			msg.manual_lockdown = actuator_armed.kill;			// Set to true if manual throttle kill switch is engaged
			msg.force_failsafe = actuator_armed.termination;			// Set to true if the actuators are forced to the failsafe position
